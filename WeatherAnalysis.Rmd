...
packages <- c("tidyverse", "lubridate", "xts", "zoo", "stringr", "dygraphs", "forecast")
...
#trend.df.temp <- ma(df.zoo$TEMP, order = 24*365.25, centre = TRUE)
#plot(as.ts(df.zoo$TEMP)) +
#  lines(trend.df.temp, col = "red")
#plot(trend.df.temp, col = "red")

#df.decomp <- decompose(df.zoo$TEMP, type = "additive")
```
