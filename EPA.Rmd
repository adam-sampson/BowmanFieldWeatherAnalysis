...
#   Check that necessary packages are installed
    packages <- c("tidyverse", "lubridate", "xts", "corrplot")
...
```{r echo=TRUE}
    ...
    par(mfrow = c(1, 2))
    plot(pm_1$Date,pm_1$Measurement , pch = 20, ylim = rng, xlab = "", 
         ylab = expression(PM[2.5] * " (" * mu * g/m^3 * ")"), col = pm_1$AQS_SITE_ID)
    abline(h = median(pm_1$Measurement, na.rm = T), col = "red")
    plot(pm_2$Date,pm_2$Measurement , pch = 20, ylim = rng, xlab = "", 
         ylab = expression(PM[2.5] * " (" * mu * g/m^3 * ")"), col = pm_2$AQS_SITE_ID)
    abline(h = median(pm_2$Measurement, na.rm = T), col = "red")
```
